// app/(auth)/(routes)/signup/page.js
"use client";
...
export default function SignUpPage() {
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      ...
      // Generate ECDH keypair and store public key in Firestore
      try {
        const { publicKey, privateJwk } = await generateECDHKeyPair();
        // store privateJwk in localStorage (client-only)
        localStorage.setItem(`ecdh_private_${user.uid}`, JSON.stringify(privateJwk));
        // save user doc with publicKey
        ...
        // fallback: create user doc without keys
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <form onSubmit={handleSignUp} className="bg-white shadow-lg rounded-2xl p-8 w-96">
        ...
        <p className="text-sm text-center mt-4 text-gray-400">Already have an account? <a href="/signin" className="text-blue-500 hover:underline">Login</a></p>
      </form>
    </div>
  );
}
